...
// Use Tampermonkey's native GM_xmlhttpRequest. A page-level fetch shim here
// would be subject to HTTPS -> localhost Private Network Access/CORS checks.
  ...
  return new Promise((resolve) => {
    ...
    const fetchPlayersInner = () => {
      searchClub({
        ...
        result.push(...response.response.items);
...
const futAutoGrind = async () => {
  if (LOCAL_REPEATABLES_ONLY) {
    createSbcGrind = false;
    showNotification(
      "Auto Grind legado desativado nesta versão local.",
      UINotificationType.NEGATIVE
    );
    return { ok: false, error: "LEGACY_AUTOMATION_DISABLED" };
  }
  try {
...
let getSBCPrice = (item, sbcId = 0, challengeId = 0) => {
  ...
  const directPrice = getPrice(item);
  const fodderPrice = getPrice({ definitionId: item.rating + "_CBR" });
  if (directPrice === -1 && fodderPrice === -1) {
    const rating = Math.max(1, Number(item.rating) || 1);
    let localCost = Math.max(100, Math.pow(Math.max(1, rating - 40), 2) * 8);
    if (item.isSpecial()) localCost += 25000;
    if (item.isTradeable()) localCost += 5000;
    ...
    return Math.max(1, Math.round(localCost));
  }
...
// Pure helpers are kept together so they can be unit-tested outside the EA page.
const AutoSbcRunnerCore = (() => {
  const normalizeMode = (rawMode, enumMap = {}) => {
    if (rawMode == null) return "";
    if (typeof rawMode === "string") return rawMode.toUpperCase();
    const enumEntry = Object.entries(enumMap || {}).find(
      ([, value]) => value === rawMode
    );
    return enumEntry ? enumEntry[0].toUpperCase() : String(rawMode);
  };

  const getRepeatabilityInfo = (set, enumMap = {}) => {
    if (!set) {
      return { repeatable: false, mode: "", remaining: 0 };
    }
    const mode = normalizeMode(
      set.repeatabilityMode ?? set._repeatabilityMode,
      enumMap
    );
    let repeatable =
      set.repeatable === true ||
      set.isRepeatable === true ||
      ["UNLIMITED", "REFRESH", "LIMITED"].includes(mode);
    if (typeof set.isRepeatable === "function") {
      try {
        repeatable = set.isRepeatable() === true || repeatable;
      } catch (_) {}
    }
    ...
    if (remaining == null) {
      ...
      const completed = Number(set.timesCompleted || 0);
      if (Number.isFinite(repeats) && repeats > 0) {
        remaining = Math.max(0, repeats - completed);
      } else {
        remaining = repeatable ? 1 : 0;
      }
    }
    return { repeatable, mode, remaining };
  };

  const validateCompletionCount = (
    value,
    repeatability,
    ...
    if (!Number.isInteger(count) || count < 1) {
      return { ok: false, error: "A quantidade deve ser um inteiro maior que zero." };
    }
    ...
    if (count > limit) {
      return {
        ok: false,
        error: `Quantidade acima do limite disponível (${limit}).`,
      };
    }
  ...
  const classifyFailure = (error) => {
    ...
    if ([429, 426, 512].includes(numericCode)) {
      return { code: numericCode, softban: true, message: "Possível softban/rate limit da EA." };
    }
    if (error?.name === "AbortError") {
      return { code: "ABORTED", softban: false, message: "Operação cancelada ou expirada." };
    }
    return {
      code: code ?? "UNKNOWN",
      softban: false,
...
let solveSBC = async (
  ...
  try {
    if (
      createSbc != true ||
      (managedBatch && repeatableBatchState.stopRequested)
    ) {
    ...
    if (sbcData == null) {
      ...
      showNotification("SBC not available", UINotificationType.NEGATIVE);
      return { ok: false, error: "SBC_NOT_AVAILABLE" };
    }
    addSbcInfo(sbcData.sbcName, sbcData.challengeName);
    if (!runOptions.skipUnassigned) {
    ...
    players.forEach((item) => {
      ...
      item.isDuplicate = duplicateIds.includes(item?.id);
    ...
    const maxSolveTime = Math.min(
      ...
      Math.max(
        1,
        Number(getSettings(sbcId, sbcData.challengeId, "maxSolveTime")) || 60
      )
    );
    ...
    showLoader(true);
    const solveAbortController = new AbortController();
    if (managedBatch) {
      repeatableBatchState.abortController = solveAbortController;
    }
    let solution;
    try {
      solution = await makePostRequest(apiUrl + "/solve", input, {
        timeoutMs: (maxSolveTime + 30) * 1000,
        signal: solveAbortController.signal,
      });
    } finally {
      if (repeatableBatchState.abortController === solveAbortController) {
        repeatableBatchState.abortController = null;
      }
    }
    ...
    try {
      const solvedPlayers = Array.isArray(solution.results)
        ? solution.results
    ...
    if (
      (runOptions.forceSubmit ||
        solution.status_code == autoSubmitId ||
        autoSubmitId == 1) &&
      autoSubmit &&
      !concepts
    ) {
      try {
        submitResponse = await sbcSubmit(_challenge, sbcSet);
        sbcSubmitted = true;
      } catch (error) {
    ...
    if (sbcSubmitted) {
      ...
      if (repeat == null) {
        const configuredRuns = Math.min(
          AUTO_SBC_REPEATABLE_MAX_COMPLETIONS,
          Math.max(
            1,
            Number(getSettings(sbcId, sbcData.challengeId, "repeatCount")) || 1
          )
        );
        repeat = configuredRuns - 1;
      }

      const totalRepeats = Math.min(
        AUTO_SBC_REPEATABLE_MAX_COMPLETIONS,
        Math.max(
          1,
          Number(getSettings(sbcId, sbcData.challengeId, "repeatCount")) || 1
      ...
      if (managedBatch) {
        ...
        return {
          ok: false,
          error: concepts ? "CONCEPT_PLAYERS_NOT_SUBMITTED" : "SUBMIT_FAILED",
          detail: submitError,
        };
      }
    ...
    const failure = AutoSbcRunnerCore.classifyFailure(err);
    return {
      ok: false,
      error: failure.softban ? "POSSIBLE_SOFTBAN" : "UNEXPECTED_ERROR",
      failure,
      detail: err,
    };
  }
...
let sbcSubmit = async function (challenge, sbcSet, i) {
  ...
  return new Promise((resolve, reject) => {
    let settled = false;
    const submitRequest = services.SBC.submitChallenge(
      challenge,
      sbcSet,
      true,
    ...
    const submitTimeout = setTimeout(() => {
      ...
      settled = true;
      try {
        submitRequest.unobserve(this);
      } catch (_) {}
      reject({
        code: "SUBMIT_TIMEOUT",
        message: "A EA não confirmou o envio em 30 segundos.",
      });
    }, 30000);
    submitRequest.observe(this, async function (obs, res) {
      if (settled) return;
      settled = true;
...
function makePostRequest(url, data, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const rejectRequest = (error, notify = true) => {
      if (settled) return;
      settled = true;
      cleanup();
      console.error(error);
    ...
    const abortFromCaller = () => {
      request?.abort();
      const error = new Error("Backend request cancelled");
      ...
      onload: (response) => {
        ...
        if (payload == null && response.responseText) {
          try {
            ...
            rejectRequest(new Error("Backend returned invalid JSON"));
            return;
          }
        }
        if (settled) return;
...
let fetchPlayerPrices = async (players) => {
  if (!ENABLE_EXTERNAL_PRICE_LOOKUPS) return;
...
const repeatableBatchDelay = (milliseconds) =>
  new Promise((resolve) => setTimeout(resolve, milliseconds));

const getRepeatableSbcSets = async () => {
  const data = await sbcSets();
  const enumMap = window.SBCRepeatabilityMode || {};
  return (data?.sets || [])
    .map((set) => ({
      set,
      info: AutoSbcRunnerCore.getRepeatabilityInfo(set, enumMap),
    }))
    .filter(({ info }) => info.repeatable && info.remaining > 0)
    .sort((left, right) =>
  ...
  try {
    for (let completion = 1; completion <= requested; completion += 1) {
      ...
      while (!cycleFinished && submittedChallenges < 20) {
        ...
        await repeatableBatchDelay(900);
        ...
        const incomplete = (challengeData?.challenges || []).filter(
          (challenge) => challenge.status !== "COMPLETED"
        );
        if (incomplete.length === 0) {
          throw {
            code: "NO_INCOMPLETE_CHALLENGE",
            message: "A EA não disponibilizou um novo desafio para esta repetição.",
          };
        }
        ...
        cycleFinished = result.finalSBC === true;
        if (!cycleFinished) {
          await repeatableBatchDelay(1500);
        }
      }
    ...
    return {
      ok: false,
      stopped:
        repeatableBatchState.stopRequested ||
        error?.code === "STOPPED" ||
    ...
    repeatableBatchState.running = false;
    repeatableBatchState.abortController = null;
    createSbc = true;
  }
};
...
let createSBCTab = async () => {
  ...
  nav.appendChild(repeatablesBtn);
...
let SOLVER_SETTINGS_KEY = "sbcSolverSettingsLocalRepeatablesV1";
      ...
      createDropDown(
        ...
        async (dropdownChallenge) => {
          ...
          createNumberSpinner(
            ...
            "Exact number of full set completions. Infinite repetition is disabled."
          );
          createToggle(
            sbcParamsTile,
...
document.addEventListener("keydown", async function onKeyR(e) {
  if (LOCAL_REPEATABLES_ONLY) return;
